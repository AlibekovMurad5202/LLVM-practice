...
+ Подготовка папок и cmake к сборке:
  ...
  ```
+ Готовим дополнительный файл подкачки (16 Гб) (без него возможно падение при сборке Clang (приблизительно на 94%) из-за недостаточного размера оперативной памяти):
  * __Внимание__: крайне рекомендуется использовать дополнительный файл подкачки. Без него возможны падения при сборке Clang (приблизительно на 94%) из-за недостаточного размера оперативной памяти
  ```bash
